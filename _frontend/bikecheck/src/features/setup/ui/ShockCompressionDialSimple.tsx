/**
 * ShockCompressionDialSimple — ovladač komprese zadního tlumiče s jediným knobem (LSC),
 * čelní pohled, průhledné pozadí. Knob je vzatý z ShockCompressionDialDual (spodní kolečko
 * LOW SPEED COMPRESSION): vyvýšený prstenec s vyleptaným názvem po obvodu a zahnutými šipkami
 * „FIRM +“ / „SOFT −“, zapuštěné čelo s ocelovým imbusem, bílá ryska ukazatele natočení.
 *
 *   <ShockCompressionDialSimple lsc={{ value: lsc, max: 20, onChange: setLsc }} />
 *
 * Hodnota = kliky od plně zavřeno. Výchozí `direction="cw"` (jako Fox): po směru hodin kliky přibývají
 * (SOFT, otevírání), proti směru ubývají až na 0 (FIRM). `direction="ccw"` obrací a šipky se přehodí.
...
export type KnobValue = {
  ...
  /** Měřítko celého knobu (výchozí 1). */
  size?: number;
  ariaLabel?: string;
};
...
/* ======================= komponenta ======================= */
// Knob stojí uprostřed plátna; šířka 320 je stejná jako u dvojité verze, takže obě kreslí stejně velký LSC.
const CX = 160, CY = 120;

export type ShockCompressionDialSimpleProps = {
  lsc: KnobValue;
  direction?: Direction;
  haptics?: boolean;
  onActive?: (knob: "lsc") => void;
  className?: string;
  style?: CSSProperties;
};

export function ShockCompressionDialSimple({ lsc, direction = "cw", haptics = true, onActive, className, style }: ShockCompressionDialSimpleProps) {
  ...
  const specs: KnobSpec[] = [
    { key: "lsc", cx: CX, cy: CY, size: lsc.size ?? 1, title: "LOW SPEED COMPRESSION", clicksPerTurn: lsc.clicksPerTurn ?? 24, ariaLabel: lsc.ariaLabel ?? "LSC, low-speed compression", onActive: () => onActive?.("lsc"), ...lsc },
  ];
  const { svgProps, knobProps } = useDialEngine(specs, direction, haptics);

  return (
    <svg viewBox="0 0 320 240" {...svgProps} className={className} role="group" aria-label="Nastavení komprese zadního tlumiče"
      ...
      <Knob spec={specs[0]} sign={sign} u={u} id={id} knobProps={knobProps("lsc")} />
    </svg>
  );
}

export default ShockCompressionDialSimple;
