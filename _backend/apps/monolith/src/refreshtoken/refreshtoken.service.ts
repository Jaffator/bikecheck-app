...
export class RefreshTokenService {
  ...
  async create(
    ...
    return this.prisma.refresh_tokens.create({
      data: { user_id, refresh_token, expires_at, user_agent, ip_address },
    });
  }
  ...
  async findByToken(refresh_token: string): Promise<refresh_tokens | null> {
    return this.prisma.refresh_tokens.findUnique({ where: { refresh_token } });
  }
  ...
  async getUserActiveTokens(user_id: number): Promise<refresh_tokens[]> {
    return this.prisma.refresh_tokens.findMany({
      where: { user_id, revoked: false, expires_at: { gt: new Date() } },
